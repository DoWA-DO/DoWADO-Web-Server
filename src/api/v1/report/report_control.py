...
from src.api.v1.report import report_service


# 로깅 및 라우터 객체 생성 - 기본적으로 추가
logger = logging.getLogger(__name__)
router = APIRouter(prefix="/report", tags=["리포트"])
...
@router.post(
    "/",
    summary="리포트 직업 추천 결과 입력",
    description="- String-Form / Text-Form / Text-Form / Text-Field",
    # response_model=CreateReport, # -> 코드 미완성, 주석처리
    responses=Status.docs(SU.CREATED, ER.NOT_FOUND)
)
async def create_report(
    report: Optional[CreateReport]
):
    await report_service.create_report(report)
    return SU.CREATED
